...
const addHighlights = (text: string, data: NERTableItem | SentimentTableItem):string =>{
...
const HighlightedDocViewer:React.FC<HighlightedDocViewerProps> = ({fileID, data, type}) => {
    ...
    let matches;
    if (type == "NER"){
        matches = new RegExp((data as NERTableItem).data.map((i,ix)=>i.text).join("|"), "g");
    } else {
  ...
  return <div className='w-full flex flex-col p-8'>
            {fileWithText && fileWithText.data && <span dangerouslySetInnerHTML={{__html: fileWithText.data.text!.replace(matches, (matched)=>{
                ...
                :
                `<span class='${sentColors[map.get(matched)! as keyof object]} whitespace-nowrap font-semibold rounded-md p-4>${matched}</span>`
            })}}></span>}
        </div>
